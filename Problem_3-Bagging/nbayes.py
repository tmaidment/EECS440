...
class NaiveBayes:
    ...
    def __init__(self, data, validationType, bins, mEstimate, training_weights):
        ...
        data_pred_pos_outcome, data_pred_neg_outcome = self._get_pos_neg_outcomes(proc_data, mEstimate, training_weights)
        ...
        self.p_pos_class = self._get_n_class(class_data, 1, training_weights) / sum(training_weights)

    def _get_pos_neg_outcomes(self, proc_data, m, training_weights):
        """
        ...
        for attr_idx in range(len(proc_data.iloc[0,:])):
            p_data_given_y0 = self._p_data_given_y(proc_data, attr_idx, class_idx, 0, m, training_weights)
            p_data_given_y1 = self._p_data_given_y(proc_data, attr_idx, class_idx, 1, m, training_weights)
    ...
    def _p_data_given_y(self, data, attr_idx, class_idx, y_value, m, training_weights):
        """
        ...
        for i in range(len(attr_data)):
            if class_data[i] == y_value:
                ...
                if not np.isnan(attr_data[i]):
                    nX_and_Y[int(attr_data[i])] += training_weights[i]
        p = 1/n_attr_vals
        # Laplace Smoothing
        if m < 0:
            m = n_attr_vals
        p_data_given_y = (nX_and_Y + m*p) / (self._get_n_class(class_data, y_value, training_weights) + m)
        return p_data_given_y


    def _get_n_class(self, class_data, class_label, training_weights):
        sum = 0
        for i in range(len(class_data)):
            if class_data[i] == class_label:
                sum += training_weights[i]
        return sum
