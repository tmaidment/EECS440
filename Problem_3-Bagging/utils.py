...
def flip_labels(dataset, p, algo):
    if algo == 'dtree':
        nCols = len(dataset[0])
        for i in range(len(dataset[0])):
            flip = random.random() < p
            if flip:
                dataset[i][nCols-1] = 1-dataset[i][nCols-1]
    else:
        nCols = len(dataset.iloc[0,:])
        for i in range(len(dataset.iloc[0,:])):
            flip = random.random() < p
            if flip:
                dataset.iloc[i, nCols-1] = 1-dataset.iloc[i, nCols-1]