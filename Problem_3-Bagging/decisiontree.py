...
class DecisionTree():
    ...
    def ensemblePrediction(self, data):
        return np.array(self.eval_set(data))

    def predict(self, data):
        output = np.array(self.eval_set(data))
        #print(np.array([output,output]).T)
        return np.array([output,output]).T
...
def build_tree(dataset, entropy_selector, maxdepth, split_crit, weights=None):
    """
